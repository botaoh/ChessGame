...
void computeMatricesFromInputsFinalProject(float _theta, float _phi, float _radius)
{
	...
	float posX = _radius * sin(glm::radians(_theta)) * cos(glm::radians(_phi));
	float posY = _radius * sin(glm::radians(_theta)) * sin(glm::radians(_phi));
	float posZ = _radius * cos(glm::radians(_theta));
